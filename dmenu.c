...
#endif // PIPEOUT_PATCH
// TODO: port for other systems
static char *cmd = "xclip -o"; /* clipboard namespace [Linux only] */
static char *embed;
static int bh, mw, mh;
#if XYW_PATCH
...
#if HIGHLIGHT_PATCH || FUZZYHIGHLIGHT_PATCH
#if EMOJI_HIGHLIGHT_PATCH
  drawhighlights(item, output + iscomment,
                 x + ((iscomment == 6) ? temppadding : 0), y, w);
#else
...
static void drawmenu(void) {
  ...
  if (passwd) {
    if (cba) {
      ...
      fp = popen(cmd, "r");
      if (fp == NULL) {
        printf("Failed to get input from clipboard");
        exit(1);
      }

      while (fgets(cb, sizeof(cb), fp) != NULL) {
        strcpy(text, cb);
      }
      pclose(fp);

      drw_text_align(drw, x, 0, curpos, bh, text, cursor, AlignR);
      drw_text_align(drw, x + curpos, 0, w - curpos, bh, "[REDACTED]",
                     10 - cursor, AlignL);
      return;
    } else {
      ...
      drw_text_align(drw, x + curpos, 0, w - curpos, bh, censort + cursor,
                     strlen(censort) - cursor, AlignL);
      free(censort);
    }
  ...
  if (lines > 0) {
    ...
    for (item = curr; item != next; item = item->right, i++)
      if (columns)
#if VERTFULL_PATCH
        drawitem(item, 0 + ((i / lines) * (mw / columns)),
                 y + (((i % lines) + 1) * bh), mw / columns);
#else
...
static void match(void) {
  ...
  for (item = items; item && (!(dynamic && *dynamic) || item->text);
       item = (dynamic && *dynamic) ? item + 1 : item->next)
...
#endif // DYNAMIC_OPTIONS_PATCH
#if HIGHPRIORITY_PATCH
/* exact matches go first, then prefixes with high priority, then prefixes, then
 * substrings */
#else
...
#elif PIPEOUT_PATCH
#if PRINTINPUTTEXT_PATCH
    if (sel && ((use_text_input && (ev->state & ShiftMask)) ||
...
static void usage(void) {
  fputs("usage: dmenu [-bv"
        ...
        "windowid]"
#if ALPHA_PATCH || BORDER_PATCH || HIGHPRIORITY_PATCH || INITIALTEXT_PATCH ||  \
    LINE_HEIGHT_PATCH || NAVHISTORY_PATCH || XYW_PATCH ||                      \
    DYNAMIC_OPTIONS_PATCH || JSON_PATCH
        "\n            "
#endif
#if JSON_PATCH
        " [ -j json-file]"
...
#endif // NAVHISTORY_PATCH
#if XYW_PATCH
        " [-X xoffset] [-Y yoffset] [-W width]" // (arguments made upper case
                                                // due to conflicts)
#endif                                          // XYW_PATCH
#if HIGHLIGHT_PATCH || FUZZYHIGHLIGHT_PATCH
        "\n [-nhb color] [-nhf color] [-shb color] [-shf color]" // highlight
                                                                 // colors
#endif // HIGHLIGHT_PATCH | FUZZYHIGHLIGHT_PATCH
        "\n",
        stderr);
  ...
  for (i = 1; i < argc; i++)
    ...
    if (!strcmp(argv[i], "-v")) { /* prints version information */
      ...
      topbar = 0;
                   ...
                   argv[i],
                   "-c")) { /* toggles centering of dmenu window on screen */
      center = !center;
#endif // CENTER_PATCH
#if !NON_BLOCKING_STDIN_PATCH
    } else if (!strcmp(argv[i],
...
#endif // PREFIXCOMPLETION_PATCH
#if FUZZYMATCH_PATCH
    } else if (!strcmp(argv[i], "-F")) { /* disable/enable fuzzy matching,
                                            depends on default */
      fuzzy = !fuzzy;
#endif // FUZZYMATCH_PATCH
#if PASSWORD_PATCH
      ...
      passwd = 1;
                       ...
                       "-Pcb")) { /* fetch an input password from clipboard */
      passwd = 1;
      cba = 1;
#endif // PASSWORD_PATCH
#if REJECTNOMATCH_PATCH
      ...
      sortmatches = 0;
                       ...
                       "-ix")) { /* adds ability to return index in list */
      print_index = 1;
#endif // PRINTINDEX_PATCH
#if RESTRICT_RETURN_PATCH
...
/* these options take one argument */
      ...
      lineheight =
          MAX(lineheight, min_lineheight); /* reasonable default in case of
